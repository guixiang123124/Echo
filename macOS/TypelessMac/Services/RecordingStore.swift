...
public actor RecordingStore {
    ...
    private init() {
        ...
        baseDirectory = appSupport.appendingPathComponent("Typeless", isDirectory: true)
        recordingsDirectory = baseDirectory.appendingPathComponent("Recordings", isDirectory: true)
        databaseURL = baseDirectory.appendingPathComponent("typeless.sqlite")
    ...
    public func saveRecording(
        ...
        if sqlite3_step(stmt) != SQLITE_DONE {
            ...
            Task { @MainActor in
                NotificationCenter.default.post(name: .typelessRecordingSaved, object: nil)
            }
        }

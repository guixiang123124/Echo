...
- 固定 Bundle ID
- 固定安装路径：`/Applications/TypelessMac.app`
...
## 验证命令
```bash
codesign -dv --verbose=4 /Applications/TypelessMac.app 2>&1 | sed -n '1,80p'
```
...
## 升级后首次清理建议（只做一次）
1. 系统设置里删除旧 Typeless 权限条目
2. 仅保留 `/Applications/TypelessMac.app` 一份
